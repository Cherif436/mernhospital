...
export const createUserController = errorHandleMiddleware(
  async (req, res, next) => {
    const { firstName, lastName, email, nic, dob, password, gender, phone } = req.body;

    if(!firstName || !lastName || !email || !nic || !dob || !password || !gender || !phone) {
      return next(new ErrorHandler("Please fill out the entire form.", 400));
    }

    const isRegistered = await User.findOne({ email });
    if(isRegistered) {
      return next(new ErrorHandler("Patient already registered. Please login", 400));
    }

    const user = await User.create({ firstName, lastName, email, nic, dob, password, gender, phone, role: "Patient" });
...
export const createAdminController = errorHandleMiddleware(
  async (req, res, next) => {
    const { firstName, lastName, email, phone, password, nic, dob, gender } = req.body;
    if(!firstName || !lastName || !email || !phone || !password || !nic || !dob || !gender) {
    ...
    const admin = await User.create({ firstName, lastName, email, phone, password, nic, dob, gender, role: "Admin" });
    res.status(201).json({
      success: true,
...
export const createNewDoctorController = errorHandleMiddleware(
  async (req, res, next) => {
    ...
    const { firstName, lastName, email, password, phone, gender, nic, dob, doctorDepartment } = req.body;
    if(!firstName || !lastName || !email || !password || !phone || !gender || !nic || !dob || !doctorDepartment || !docImage) {
      return next(new ErrorHandler("PLease Fill Full Form", 400));
    }
    ...
    // code for cloudinary
    const cloudinaryResponse = await cloudinary.uploader.upload(docImage.tempFilePath);
...
// logOut admin
export const logOutAdmin = errorHandleMiddleware(
  async (req, res, next) => {
    res
      .status(200)
...
// logOut patient
export const logOutPatient = errorHandleMiddleware(
  async (req, res, next) => {
    res
      .status(200)
...
// logOut doctor
export const logOutDoctor = errorHandleMiddleware(
  async (req, res, next) => {
    res
      .status(200)
